...
export function ChatWindow() {
  ...
  return (
    <div className="flex flex-col h-screen max-w-4xl mx-auto p-4 text-white">
      <h1 className={`text-2xl font-semibold mb-4 ${spaceGrotesk.className}`}>GoNoGo</h1>
      <div className="flex-1 overflow-y-auto border border-white/5 shadow-lg rounded-3xl p-4 bg-slate-900/50 backdrop-blur-sm">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-slate-400">
            ...
            {SUGGESTIONS.map((s) => (
              <button
                ...
                className="border border-slate-700 rounded-full px-4 py-2 text-sm hover:bg-slate-800 transition-colors"
              >
                {s}
              </button>
            ))}
      ...
      <div className="flex mt-4 gap-2">
        <input
          className="flex-1 border border-slate-700 bg-slate-800 text-white rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-amber-500"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && handleSend()}
          placeholder="Describe your outdoor plan, or ask anything..."
        />
        <button onClick={() => handleSend()} className="bg-amber-500 hover:bg-amber-600 transition-colors text-white rounded-full px-6 py-2 shadow-sm font-medium">
          Send
        </button>
      </div>
    </div>
